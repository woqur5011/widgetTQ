...
class Ma200RefreshCallback : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        try {
            Log.d("WITTQ_MA200_DEBUG", "Refresh button clicked")
            Ma200Widget().updateAll(context)
            Log.d("WITTQ_MA200_DEBUG", "Widget Refresh completed")
        } catch (e: Exception) {
            Log.e("WITTQ_MA200_DEBUG", "Widget Refresh failed: ${e.message}", e)
        }
    }
}
...
class Ma200Widget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        Ma200UpdateWorker.enqueue(context)
        ...
        val resultData = withContext(Dispatchers.IO) {
            try {
                val marketData = MarketDataEngine.fetchMarketData("TQQQ") ?: return@withContext null
                        ...
                        Box(
                            ...
                            Image(
                                ...
                                modifier = GlanceModifier.size((15 * factor).dp)
                                    .clickable(actionRunCallback<Ma200RefreshCallback>())
                            )
                        }
                    }
                }
